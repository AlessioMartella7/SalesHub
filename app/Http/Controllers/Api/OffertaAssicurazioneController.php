...
class OffertaAssicurazioneController extends Controller
{
    ...
    public function index()
    {
        ...
        if (filled(request('codice_contratto')) || filled(request('codice_pdv'))){
            $offertaAssicurazione = $query->orderBy('id', 'asc')->get();
        } else {
            $offertaAssicurazione = $query->orderBy('id', 'asc')->paginate(200);
        }

        return response()->json($offertaAssicurazione, 200);
    }
    ...
    public function checkLastUpdatedDate(Request $request){
        ...
        $ultimaDataAggiornata = OffertaAssicurazione::whereIn('codice_pdv', $codici)
